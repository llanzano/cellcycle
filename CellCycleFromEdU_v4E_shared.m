...
%$£  Anna Provvidenza Privitera1#, Silvia Scalisi1#, Greta Paternò1, Elena Cerutti1,2, Morgana D’Amico1, Pier Giuseppe Pelicci3,4, Mario Faretta3, Gaetano Ivan Dellino3,4,*, Alberto Diaspro2,5, Luca Lanzanò1,2,*
...
function [ ]=CellCycleFromEdU_v4E_shared(filename );

% goal
% 
% count pixel/intensity in edu channel (must be first channel)
% load: count mask nuclei (load 2x, temporary to be fixed), edu mask, intensity image 
% open stacks
% v4 during revision
...
kslopeE=0.2;
kslopeL=0.2;
minpx=20;  % minimum pixels for EdU signal
SizeLimit=1.3;
...
Thrmask=0;
% MinPxPos=0; % min pixels in 2nd mask to detect positive cell CHECK..
...
%     Center1=0;
    ...
    SpotCell=unique(Mask1(Mask1>0));   %%% DA SISTEMARE!!!! anche ch2
    N1all=numel(SpotCell);
 
    for kidx=1:N1all
    spotidx=SpotCell(kidx);
    Size1 =   numel(find(Mask1==spotidx ))  ; %CHECK
    SizeinROI =   numel(find(Mask1==spotidx & MaskROI>0 ) )  ; %CHECK
    if SizeinROI > Size1*0.5
    spot=spot+1; 
    [row,col] = find(Mask1==spotidx);
    Center1(kidx,:) = [ mean(row) ,  mean(col) ] ;
    ResultTemp1=cat(2, spot, Size1, Center1(kidx,:), cell, ifile, istack, double(cellidx), double(spotidx) ) ;
    ResultSpot1=cat(1, ResultSpot1, ResultTemp1); % must initialize
    end
    end
    N1=spot-spotfirst+1;
    if N1==0
    ...
    SizeAv1=mean(ResultSpot1(spotfirst:spotfirst+N1-1,2));
    end
end

for i=1
if doublech==1
%channel 2 object analysis (sistemare come ch1 con mask) ridurre tempi di
%analisi
if N2max==0
    N2=0;
    SizeAv2=0;
%     Center1=0;
    ...
    SpotCell2=unique(Mask2(Mask2>0));
    N2=numel(SpotCell2);
    spotfirst2=spot2+1;
    for kidx=1:N2
    spotidx2=SpotCell2(kidx);
    spot2=spot2+1;    
    Size2 =   numel(find(Mask2==spotidx2 ))  ; %CHECK
    [row,col] = find(Mask2==spotidx2);
    Center2(kidx,:) = [ mean(row) ,  mean(col) ] ;
    ResultTemp2=cat(2, spot2, Size2, Center2(kidx,:), cell, ifile,istack, double(cellidx), double(spotidx2) ) ;
    ResultSpot2=cat(1, ResultSpot2, ResultTemp2); % must initialize
    end
    SizeAv2=mean(ResultSpot2(spotfirst2:spotfirst2+N2-1,2));
end
...
if     ~(N1==0 || N2==0)
    for kidx1=1:N1
       for kidx2=1:N2
          dist=dist+1; % counter
          distraw = Center1(kidx1,:) - Center2(kidx2,:)  ;
          d = abs(complex(distraw(1), distraw(2)));
          if d < distcoloc
              Ncoloc=Ncoloc+1;
          end
          Size1=ResultSpot1( spotfirst-1 +kidx1  , 2);
          Size2=ResultSpot2( spotfirst2-1 +kidx2  , 2);
          ResultDTemp=cat(2, dist, d, Size1, Size2, cell, ifile,istack, double(cellidx) ) ;
          ResultDist=cat(1, ResultDist, ResultDTemp); % must initialize
       end
    end
%     dav=mean(ResultSpot2(spotfirst2:spotfirst2+N2-1,2));
end
else
N2=N1;
SizeAv2=SizeAv1;
...
% cearly=(c1&~c2&c3 & csize) ; 
% cearlylarge=(c1&~c2&c3 & ~csize) ; 
% clate=(c1&~c2&c4) ; % | cearlylarge ;
% % cunclass=c1&~c2 &(~cearly &~c4 )  ;
% 
% %border cells
% 
% if isempty(min(Intedu(clate>0)))==0  && isempty(min(Intedu(cearly>0)))==0 && isempty(max(densedu(c1==0)))==0
% celllateb=find(Intedu==min(Intedu(clate>0&cearlylarge==0)),1);
% cellearlyb=find(Intedu==max(Intedu(cearly>0)),1);
% cellGb=find(densedu==max(densedu(c1==0)),1);
% subplot(2,2,1)
% imagesc(  Imgeducrop {cellearlyb}  ) ;
% title('EARLY S')
% colormap(greenmap);
...
% axis off
% end
% prompt2 = {'Minimum number of EdU pixels for S phase:', 'Density Threshold for Mid (relative to maximum):','Intensity Threshold for Early (relative to minimum):', 'Intensity Threshold for Late (relative to minimum):','Maximum size for Early (relative to average):', 'file name'}; 
% dlg_title2 = 'Input parameters'; 
% num_lines = 1;
% def2 = {num2str(minpx),num2str(densfactor),num2str(IntFactor1),num2str(IntFactor2), num2str(SizeLimit),tag};
% answer2 = inputdlg(prompt2,dlg_title2,num_lines,def2);
% figcheck=~isempty(answer2); 
% if figcheck==1
% minpx=str2num(answer2{1});
% densfactor=str2num(answer2{2});
% IntFactor1=str2num(answer2{3});
% IntFactor2=str2num(answer2{4});
% SizeLimit=str2num(answer2{5});
% tag=answer2{6};
% end
% end
